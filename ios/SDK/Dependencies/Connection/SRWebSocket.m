...
- (void)setAllowInsecureConnections:(BOOL)allowInsecureConnections
{
    _allowInsecureConnections = allowInsecureConnections;

    NSMutableDictionary *SSLOptions = [[_outputStream propertyForKey:(__bridge id)kCFStreamPropertySSLSettings] mutableCopy];
    [SSLOptions setValue:[NSNumber numberWithBool:allowInsecureConnections ? NO : YES] forKey:(__bridge id)kCFStreamSSLValidatesCertificateChain];
    [_outputStream setProperty:SSLOptions
                        forKey:(__bridge id)kCFStreamPropertySSLSettings];
}
...
- (void)_handleMessage:(id)message
{
    ...
    [self _performDelegateBlock:^{
        ...
        else if ([self.delegate respondsToSelector:@selector(webSocket:didReceiveMessage:)])
        {
