//
//  MessagePackPacker.m
//  Fetch TV Remote
//
//  Created by Chris Hulbert on 13/10/11.
//  Copyright (c) 2011 Digital Five. All rights reserved.
//

//##OBJCLEAN_SKIP##

#import "MessagePackPacker.h"
#include "msgpack_src/msgpack.h"

@implementation MessagePackPacker

// dates are always sent over the wire with an en_US locale and the UTC time zone
// to ensure both sender/receiver have the same formatting. upon receipt they are
// localized as appropriate with a seperate date formatter

+ (NSDateFormatter *)sharedDateFormatter
{
    static NSDateFormatter *dateFormatter = nil;
    
    if (!dateFormatter)
    {
        @synchronized (self)
        {
            if (!dateFormatter)
            {
                dateFormatter = [[NSDateFormatter alloc] init];
                // this is dumb, because 10.8SDK doesn't make +[NSLocale localeWithLocaleIdentifier:] public, when we start building with 10.9 SDK we can get rid of this
                [dateFormatter setLocale:[NSLocale performSelector:@selector(localeWithLocaleIdentifier:) withObject:@"en_US"]];
                [dateFormatter setTimeZone:[NSTimeZone timeZoneWithName:@"UTC"]];
                [dateFormatter setDateFormat:@"MMM d, yyyy hh:mm:ss a"];
            }
        }
    }
    
    return dateFormatter;
}

// this date formatter is used to localize dates when returned to the client

+ (NSDateFormatter *)sharedLocalDateFormatter
{
    static NSDateFormatter *localDateFormatter = nil;

    if (!localDateFormatter)
    {
        @synchronized (self)
        {
            if (!localDateFormatter)
            {
                localDateFormatter = [[NSDateFormatter alloc] init];
                [localDateFormatter setLocale:[NSLocale currentLocale]];
                [localDateFormatter setTimeZone:[NSTimeZone defaultTimeZone]];
                [localDateFormatter setDateFormat:@"MMM d, yyyy hh:mm:ss a"];
            }
        }
    }

    return localDateFormatter;
}

// Pack a single number, figuring out which type of number it is
+ (void)packNumber:(NSNumber*)num into:(msgpack_packer*)pk {
#ifdef GNUSTEP
        const char* type = [num objCType];
        if(strcmp(type,@encode(int8_t)) == 0)
        {
          msgpack_pack_int8(pk, num.shortValue);
        }
        else if(strcmp(type,@encode(int16_t)) == 0 ||
                strcmp(type,@encode(short)) == 0)
        {
          msgpack_pack_int16(pk, num.shortValue);
        }
        else if(strcmp(type,@encode(int32_t)) == 0 ||
                strcmp(type,@encode(int)) == 0 ||
                strcmp(type,@encode(long)) == 0)
        {
          msgpack_pack_int32(pk, num.intValue);
        }
        else if(strcmp(type,@encode(int64_t)) == 0 ||
                strcmp(type,@encode(long long)) == 0)
        {
          msgpack_pack_int64(pk, num.longLongValue);
        }
        else if(strcmp(type,@encode(float)) == 0)
        {
          msgpack_pack_float(pk, num.floatValue);
        }
        else if(strcmp(type,@encode(double)) == 0)
        {
          msgpack_pack_double(pk, num.doubleValue);
        }
        else if(strcmp(type,@encode(char)) == 0 ||
                strcmp(type,@encode(BOOL)) == 0)
        {
          int theValue = num.intValue;
          if (theValue == 0)
            msgpack_pack_false(pk);
          else if (theValue == 1)
            msgpack_pack_true(pk);
          else
            msgpack_pack_int16(pk, theValue);
        }
        else
        {
            NSLog(@"Could not messagepack number, cannot recognise type: %s", type);
        }
#else
	CFNumberType numberType = CFNumberGetType((CFNumberRef)num);
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wcovered-switch-default"
	switch (numberType)	{
		case kCFNumberSInt8Type:
			msgpack_pack_int8(pk, num.shortValue);
			break;
		case kCFNumberSInt16Type:
		case kCFNumberShortType:
			msgpack_pack_int16(pk, num.shortValue);
			break;
		case kCFNumberSInt32Type:
		case kCFNumberIntType:
		case kCFNumberLongType:
		case kCFNumberCFIndexType:
		case kCFNumberNSIntegerType:
			msgpack_pack_int32(pk, num.intValue);
			break;
		case kCFNumberSInt64Type:
		case kCFNumberLongLongType:
			msgpack_pack_int64(pk, num.longLongValue);
			break;
		case kCFNumberFloat32Type:
		case kCFNumberFloatType:
		case kCFNumberCGFloatType:
			msgpack_pack_float(pk, num.floatValue);
			break;
		case kCFNumberFloat64Type:
		case kCFNumberDoubleType:
			msgpack_pack_double(pk, num.doubleValue);
			break;
		case kCFNumberCharType: {
			int theValue = num.intValue;
			if (theValue == 0)
				msgpack_pack_false(pk);
			else if (theValue == 1)
				msgpack_pack_true(pk);
			else
				msgpack_pack_int16(pk, theValue);
		}
			break;
		default:
			NSLog(@"Could not messagepack number, cannot recognise type: %@", num);
	}
#pragma clang diagnostic pop
#endif
}

// Pack a single object into the given packer
+ (void)packObject:(id)obj into:(msgpack_packer*)pk {
	if ([obj isKindOfClass:[NSArray class]]) {
		msgpack_pack_array(pk, (unsigned int)((NSArray*)obj).count);
		for (id arrayElement in obj) {
			[self packObject:arrayElement into:pk];
		}
    } else if ([obj isKindOfClass:[NSSet class]]) {
        NSArray *array = [obj allObjects];
        msgpack_pack_array(pk, (unsigned int)array.count);
        for (id arrayElement in array) {
            [self packObject:arrayElement into:pk];
        }
	} else if ([obj isKindOfClass:[NSDictionary class]]) {
		msgpack_pack_map(pk, (unsigned int)((NSDictionary*)obj).count);
		for(id key in obj) {
			[self packObject:key into:pk];
			[self packObject:[obj objectForKey:key] into:pk];
		}
	} else if ([obj isKindOfClass:[NSString class]]) {
		const char *str = ((NSString*)obj).UTF8String;
		unsigned int len = (unsigned int)strlen(str);
		msgpack_pack_raw(pk, len);
		msgpack_pack_raw_body(pk, str, len);
    } else if ([obj isKindOfClass:[NSDate class]]) {
        NSDateFormatter *dateFormatter = [MessagePackPacker sharedDateFormatter];
        NSString *dateString = [dateFormatter stringFromDate:obj];
		const char *str = ((NSString*)dateString).UTF8String;
		unsigned int len = (unsigned int)strlen(str);
		msgpack_pack_raw(pk, len);
		msgpack_pack_raw_body(pk, str, len);
	} else if ([obj isKindOfClass:[NSNumber class]]) {
		[self packNumber:obj into:pk];
	} else if (obj==[NSNull null]) {
		msgpack_pack_nil(pk);
	} else {
		NSLog(@"Could not messagepack object: %@", obj);
	}
}

// Given an array or dictionary, this messagepacks it
+ (NSData*)pack:(id)obj {
	// Creates buffer and serializer instance
	msgpack_sbuffer* buffer = msgpack_sbuffer_new();
	msgpack_packer* pk = msgpack_packer_new(buffer, msgpack_sbuffer_write);
	
	// Pack the root array or dictionary node, which recurses through the rest
	[self packObject:obj into:pk];
	
	// Bridge the data back to obj-c's world
	NSData* data = [NSData dataWithBytes:buffer->data length:buffer->size];
	
	// Free
	msgpack_sbuffer_free(buffer);
	msgpack_packer_free(pk);
	
	return data;
}

@end

//##OBJCLEAN_ENDSKIP##
